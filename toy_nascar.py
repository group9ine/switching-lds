...
def stupid_model(T):
    ...
    for t in range(1,T+1):
        theta=t*3*2*np.pi/T
        if (np.sin(theta)>0 and np.cos(theta)>0) or (np.sin(theta)<0 and np.cos(theta)<0):
...
def advanced_model(T): ## don't use yet
    ...
    for t in range(T):
        ...
        f_r_dist = get_track_limits(x[-1], [(v[0]+v[1])/2, (v[1]-v[0])/2])
        f_l_dist = get_track_limits(x[-1], [(v[0]-v[1])/2, (v[1]+v[0])/2])
