...
model {
  ...
  // prior on x[1] and uniform prior log p(z[1] = k)
  gamma[1] = rep_vector(multi_normal_prec_lpdf(x[1] | mu, Sigma) - log(K), K);
  // log-likelihood p(y[1] | z[1] = k, x[1])
  for (k in 1:K)
    gamma[1, k] += multi_normal_prec_lpdf(y[1] | C[k] * x[1] + d[k], S[k]);
